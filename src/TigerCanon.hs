...
data Obus = O {mapLS :: Mapper Tank Label [Stm], lgen :: Int , tgen :: Int}

firstTank :: Obus
firstTank = O {mapLS = emptyI, lgen = 0, tgen = 0}
...
instance Trackable Tank where
    enterBlock' l b = do
        st <- get
        let nmap = insertI l b (mapLS st)
        put st{mapLS=nmap}
    getBlock l = do
        st <- get
        return $ lookupI l (mapLS st)
...
traceR b@(Label lab : _)  rs = do
    ...
    case splitlast b of
        ...
        (most, Jump _ _) -> do {t <- getnext rs; return $ b ++ t}
